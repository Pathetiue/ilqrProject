...
def ilqr(x_in, u_in, iterations):
    ...
    while itr < iterations:
        ...
        for k in range(N-2, -1, -1):
            ...
            fx = substitute([A], [theta, w, p, v, F], 
                            x_traj[:, k].flatten().tolist() + u_traj[:, k].tolist())[0]
            fx = cas2arr(fx)
            fu = substitute([B], [theta, w, p, v, F], 
                            x_traj[:, k].flatten().tolist() + u_traj[:, k].tolist())[0]
            fu = cas2arr(fu)
            # Compute Q functions
        ...
        while new_cost is None or new_cost > prev_cost:
            # Forward pass
            for k in range(N-1):
...
if __name__ == "__main__":
    ...
    np.save("states_p{}.npy".format(pred_horiz), x_res)
    np.save("u_res_p{}.npy".format(pred_horiz), u_res)
